...
const solutions = [
  {
    ...
    tags: ["LV Systems", "Generators", "ATS", "Power"], href: "/solutions/electrical-engineering"
  },
  {
    title: "Power & Energy",
    text: "Solar, energy monitoring and resilient power solutions designed around reliability, operating cost and continuity.",
    tags: ["Solar", "Energy Monitoring", "Backup Power"], href: "/solutions/power-energy"
  },
  {
    ...
    tags: ["BMS", "BACnet", "KNX", "HVAC"], href: "/solutions/building-automation"
  },
  {
    title: "Industrial Automation",
    text: "Control, monitoring and industrial communications for plants, production environments and critical processes.",
    tags: ["PLC", "SCADA", "Instrumentation"], href: "/solutions/industrial-automation"
  },
  {
    ...
    tags: ["CCTV", "Access Control", "Intrusion"], href: "/solutions/electronic-security"
  },
  {
    ...
    tags: ["Integration", "BACnet/IP", "Modbus", "IP"], href: "/solutions/systems-integration"
  },
  {
    title: "Fire & Life Safety",
    text: "Fire detection, alarm interfaces and life-safety systems integrated with wider facility infrastructure.",
    tags: ["Fire Detection", "Alarm Interfaces", "Emergency Systems"], href: "/solutions/fire-life-safety"
  }
];

const industries = [
  ["Mining & Resources", "Resilient power, monitoring, security and automation for demanding operating environments.", "/industries/mining"],
  ["Manufacturing", "Electrical infrastructure, automation, controls and monitoring for production facilities.", "/industries/manufacturing"],
  ["Commercial Buildings", "Integrated electrical, BMS, security and energy systems for modern facilities.", "/industries/commercial"],
  ["Healthcare", "Critical power, life safety, security and building systems where reliability matters.", "/industries/healthcare"],
  ["Education", "Campus-wide power, connectivity, security, automation and monitoring solutions.", "/industries/education"],
  ["Hospitality", "Guest safety, access, BMS and energy systems engineered around operational efficiency.", "/industries/hospitality"]
];
...
export default function HomePage() {
  ...
  async function submitForm(event) {
    ...
    setFormStatus("Sending your project enquiry…");
    try {
      ...
      setFormStatus("Your project enquiry has been sent. Our team will be in touch.");
      form.reset();
    } catch (error) {
      console.error(error);
      setFormStatus(error.message || "Unable to send enquiry right now.");
    } finally {
          ...
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-border-light border border-border-light">
              {solutions.map((solution) => (
                <Link key={solution.title} href={solution.href} className="bg-primary-bg p-8 lg:p-9 hover:bg-secondary-bg transition-colors">
                  ...
                  <span className="mt-6 inline-block font-semibold text-accent-red">Explore capability →</span>
                </Link>
              ))}
            </div>
          </div>
        ...
        <section id="industries" className="bg-primary-text text-white py-24">
          <div className="max-w-7xl mx-auto px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-10">
              {industries.map(([title, text, href]) => <Link href={href} key={title} className="border-t border-white/20 pt-6 hover:text-red-300"><h3 className="text-xl font-bold mb-3">{title}</h3><p className="text-white/60 leading-relaxed">{text}</p><span className="mt-4 inline-block text-sm font-semibold">View sector capability →</span></Link>)}
            </div>
          </div>
        </section>
        ...
        <section id="contact" className="bg-primary-text text-white py-24">
          <div className="max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-16">
            ...
            <form onSubmit={submitForm} className="bg-white text-primary-text p-7 md:p-9 grid gap-4" aria-label="Project enquiry form">
              ...
              <div className="grid md:grid-cols-2 gap-4"><div className="grid gap-1"><label htmlFor="name">Your name *</label><input id="name" name="name" required minLength={2} maxLength={100} autoComplete="name" className="border border-border-light px-4 py-3 rounded-md" /></div><div className="grid gap-1"><label htmlFor="company">Company</label><input id="company" name="company" maxLength={150} autoComplete="organization" className="border border-border-light px-4 py-3 rounded-md" /></div></div>
              <div className="grid md:grid-cols-2 gap-4"><div className="grid gap-1"><label htmlFor="email">Email *</label><input id="email" name="email" type="email" required maxLength={254} autoComplete="email" className="border border-border-light px-4 py-3 rounded-md" /></div><div className="grid gap-1"><label htmlFor="phone">Phone / WhatsApp</label><input id="phone" name="phone" maxLength={50} autoComplete="tel" className="border border-border-light px-4 py-3 rounded-md" /></div></div>
              <div className="grid md:grid-cols-2 gap-4"><div className="grid gap-1"><label htmlFor="projectType">Project type</label><select id="projectType" name="projectType" className="border border-border-light px-4 py-3 rounded-md"><option value="">Select a project type</option>{['Electrical Engineering','Power & Energy','Generator / ATS','BMS / Building Automation','Industrial Automation','Electronic Security','Fire & Life Safety','Systems Integration','Other'].map(x => <option key={x}>{x}</option>)}</select></div><div className="grid gap-1"><label htmlFor="projectStage">Project stage</label><select id="projectStage" name="projectStage" className="border border-border-light px-4 py-3 rounded-md"><option value="">Select a project stage</option>{['Concept','Design','Tender','Construction','Existing Facility Upgrade','Maintenance','Other'].map(x => <option key={x}>{x}</option>)}</select></div></div>
              <div className="grid gap-1"><label htmlFor="projectLocation">Project location</label><input id="projectLocation" name="projectLocation" maxLength={150} autoComplete="address-level2" className="border border-border-light px-4 py-3 rounded-md" /></div>
              <div className="grid gap-1"><label htmlFor="message">Project requirement *</label><textarea id="message" name="message" required minLength={10} maxLength={5000} rows={6} placeholder="Tell us about the project, problem or requirement" className="border border-border-light px-4 py-3 rounded-md" /></div>
              ...
              <p role="status" aria-live="polite" className="min-h-6 text-sm">{formStatus}</p>
            </form>
          </div>
        </section>
      </main>
